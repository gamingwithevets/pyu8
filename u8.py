...
class U8:
	def __init__(self, rom: bytes, ram_size: int, rom_window_size: int) -> None:
		...
		self.hwi_ptrs = [ctypes.c_ushort(self.concat_bytes(self.rom_w[i:i+2])) for i in range(0xa, 0x7f, 2)]
		self.swi_ptrs = [ctypes.c_ushort(self.concat_bytes(self.rom_w[i:i+2])) for i in range(0x80, 0xff, 2)]
	...
	def read_mem(self, addr: int, bytes_to_fetch: int = 1) -> int: return self.rom[addr:addr+bytes_to_fetch]

	def er(self, n: int) -> int:
		if n % 2 != 0: logging.warning(f'tried to fetch invalid ER register number ({n})')
		else: return self.concat_bytes(self.r[n+1].value, self.r[n].value)
	def xr(self, n: int) -> int:
		if n % 4 != 0: logging.warning(f'tried to fetch invalid XR register number ({n})')
		else: return self.concat_bytes(self.er(n), self.er(n+1))
	def qr(self, n: int) -> int:
		if n % 8 != 0: logging.warning(f'tried to fetch invalid QR register number ({n})')
		else: return self.concat_bytes(self.xr(n), self.xr(n+1))
	def ecr(self, n: int) -> int:
		if n % 2 != 0: logging.warning(f'tried to fetch invalid ECR register number ({n})')
		else: return self.concat_bytes(self.cr[n+1].value, self.cr[n].value)
	def xcr(self, n: int) -> int:
		if n % 4 != 0: logging.warning(f'tried to fetch invalid XCR register number ({n})')
		else: return self.concat_bytes(self.ecr(n), self.ecr(n+1))
	def qcr(self, n: int) -> int:
		if n % 8 != 0: logging.warning(f'tried to fetch invalid QCR register number ({n})')
	...
	def set_psw(self, c: int = 0, z: int = 0, s: int = 0, ov: int = 0, mie: int = 0, hc: int = 0, elevel: int = 0) -> None:
		psw_str = f'0b{c}{z}{s}{ov}{mie}{hc}{"0" if elevel < 2 else ""}{str(elevel)[2:]}'
		self.psw.value = int(psw_str, 2)

	def exec_instruction(self) -> None:
		def warn(): logging.warning(f'unknown instruction code {format(self.concat_bytes(ins_code), "04X")} at address {format(self.csr.value, "02X")}:{format(pc, "04X")}')

		pc = self.pc.value
		csr_pc = self.concat_bytes(self.csr.value, pc)
		ins_code = self.read_mem(csr_pc, 2)
		self.pc.value += 2
		psw = self.fmt_psw()
		...
		# ___ Rn, Rm
		if self.get_bits(ins_code[0], 4, 4) == 8:
		...
		# ___ ERn, ERm
			...
			ern = self.er(self.get_bits(ins_code[0], 4))
		...
		# Bcond Radr/BC cond, Radr
			...
			if ins_code[1] == 0:
				...
				self.pc.value = self.read_mem(csr_pc + 2, 2)
			# BL Cadr
			if ins_code[1] == 1:
				self.lr.value = csr_pc + 4
			...
			# BL ERn
				...
				self.pc.value = self.er(self.get_bits(ins_code[1], 4, 4))
				self.lr.value = csr_pc + 2
				self.lcsr.value = self.csr.value
		# CPLC
		elif ins_code == 'b\xfe\xcf': self.set_psw(int(not self.fmt_psw['c']))
		# BRK
	...
	def cmpc(self, op1: int, op2: int):
		...
		result = ctypes.c_byte(op2 - op1 - psw['c']).value
		carry = (result >> 15) & 1
		overflow = result < op1
		half_carry = (result >> 3) & 1 or (result & (1 << 3)) >> 3
